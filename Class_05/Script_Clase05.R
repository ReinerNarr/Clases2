...
covid<-fread(input ="Class_05/2020-04-08-CasosConfirmados.csv") # función más rápida para cargar bases grandes. Si son formato csv, es la mejor forma de cargarlo.
str(covid) # me fijo que casos confirmados están en tipo character
sapply(covid,FUN = class)

covid[,`Casos Confirmados`:=as.numeric(`Casos Confirmados`)] # cambiamos casos confirmados a numéricos
...
help(package='chilemapas') # es un paquete de datos y mapas, no de funciones
...
library(RColorBrewer) # paquete de colores
brewer.pal.info
paleta <- rev(brewer.pal(n = 5,name = "Reds")) # n=5 son la cantidad de colores que tiene mi paleta. Rev es para que colores comiencen al reves.
...
# variable la divido en 5 clases
...
st_crs(comunas_rm)
comunas_rm<-st_transform(comunas_rm,crs = "+proj=longlat +datum=WGS84") # leaflet funciona en google maps y funciona en la unidad de medida entre corchetes
